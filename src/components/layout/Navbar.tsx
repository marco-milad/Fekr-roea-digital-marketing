    ...
    <header className="fixed top-0 right-0 left-0 z-50 bg-background/95 backdrop-blur-sm border-b border-border shadow-sm">
      <div className="container mx-auto px-3 sm:px-4 md:px-6 lg:px-8">
        <div className="flex flex-row-reverse h-16 sm:h-18 md:h-20 items-center justify-between gap-3 sm:gap-4 md:gap-6 xl:gap-8">
          ...
          {/* Desktop Navigation - يظهر من 1100px وفوق */}
          <nav className="hidden xl:flex items-center gap-4 2xl:gap-6 flex-1" role="navigation" aria-label="القائمة الرئيسية">
            {navLinks.map((link) => (
              <div 
                key={link.label} 
                className="relative" 
                ...
                {link.hasDropdown ? (
                  <>
                    <button
                      ...
                      className={cn(
                        "flex items-center gap-1 text-sm 2xl:text-base font-medium transition-colors hover:text-primary py-2 whitespace-nowrap",
                    ...
                    {openDropdown === link.label && link.subLinks && (
                      <div 
                        ...
                        role="menu"
                        className="absolute top-full right-0 mt-2 w-56 bg-background border border-border rounded-lg shadow-lg py-2 z-50 animate-in fade-in slide-in-from-top-2 duration-200"
                      >
                  ...
                  <Link
                    to={link.href}
                    className={cn(
                      "text-sm 2xl:text-base font-medium transition-colors hover:text-primary py-2 relative after:absolute after:bottom-0 after:right-0 after:h-0.5 after:w-0 after:bg-primary after:transition-all hover:after:w-full whitespace-nowrap",
          ...
          <div className="hidden xl:flex items-center gap-3 2xl:gap-4">
            ...
            <Link
              to="/contact"
              className="bg-primary text-primary-foreground px-4 2xl:px-5 py-2 2xl:py-2.5 rounded-lg font-medium text-sm hover:bg-primary/90 transition-all hover:shadow-md active:scale-95 whitespace-nowrap"
            >
              احصل على عرض
            </Link>
          </div>

          {/* Tablet/Medium CTA Only - من 768px لـ 1100px */}
          <div className="hidden md:flex xl:hidden">
            <Link
              to="/contact"
              className="bg-primary text-primary-foreground px-4 md:px-5 py-2 md:py-2.5 rounded-lg font-medium text-sm hover:bg-primary/90 transition-all hover:shadow-md active:scale-95 whitespace-nowrap"
            >
              تواصل معنا
            </Link>
          </div>
          ...
          <button
            onClick={toggleMenu}
            className="xl:hidden p-2 text-foreground hover:bg-muted rounded-md transition-colors flex-shrink-0"
        ...
        {isMenuOpen && (
          <div 
            id="mobile-menu"
            className="xl:hidden py-4 border-t border-border animate-in slide-in-from-top duration-200 max-h-[calc(100vh-5rem)] overflow-y-auto"
          >
            <nav className="flex flex-col gap-1" role="navigation" aria-label="القائمة الرئيسية المحمول">
                  ...
                  {link.hasDropdown ? (
                    <>
                      <button
                        onClick={() => handleDropdownToggle(link.label)}
                        className={cn(
                          "flex items-center justify-between w-full font-medium transition-colors hover:text-primary py-3 px-3 rounded-md hover:bg-muted/50 text-sm sm:text-base",
                      ...
                      {openDropdown === link.label && link.subLinks && (
                        <div 
                          ...
                          role="menu"
                          className="pr-4 border-r-2 border-primary mr-3 mt-1 animate-in slide-in-from-top-2 duration-200"
                        >
                          {link.subLinks.map((subLink) => (
                            <Link
                              key={subLink.href}
                              ...
                              className="block py-2.5 px-3 text-sm text-muted-foreground hover:text-primary transition-colors rounded-md hover:bg-muted/30"
                            >
                              {subLink.label}
                            </Link>
                          ))}
                    ...
                    <Link
                      ...
                      className={cn(
                        "block font-medium transition-colors hover:text-primary py-3 px-3 rounded-md hover:bg-muted/50 text-sm sm:text-base",
              ...
              <div className="mt-4 pt-4 border-t border-border">
                <p className="text-xs text-muted-foreground mb-3 px-3">تابعنا على:</p>
                <div className="flex items-center justify-center gap-4 px-3">
                  <a 
                    href="https://facebook.com" 
                    target="_blank" 
                    rel="noopener noreferrer"
                    className="flex h-10 w-10 items-center justify-center rounded-lg bg-muted text-foreground hover:bg-primary hover:text-primary-foreground transition-all"
                    aria-label="Facebook"
                  >
